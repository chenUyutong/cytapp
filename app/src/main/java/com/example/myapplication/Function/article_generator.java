...
public class article_generator extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_article_generator);
        ...
        btnOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String content = Generate_articles(edtTheme.getText().toString(), wenzanzhishu);
...
//    public static String 主题 = "666";
    ...
    public static int wenzanzhishu = 4000;
    ...
    public static String[] famous_quotes = {
    ...
    public static String[] back_talk = {
            "这不禁令我深思。 ",
    ...
    public static String[] front_talk = {
            "曾经说过",
            "在不经意间这样说过"
    };

    public static String take_a_word(String[] 列表) {
        int 坐标 = (int) Math.floor(Math.random() * 列表.length);
        return 列表[坐标];
    }

    public static int take_any_number(Integer 最小值, Integer 最大值) {
        if (最小值 == null) {
            最小值 = 0;
        }
        if (最大值 == null) {
    ...
    public static String 来点famous_quotes() {
        String 名言 = take_a_word(famous_quotes);
        名言 = 名言.replace("曾经说过", take_a_word(front_talk));
        名言 = 名言.replace("这不禁令我深思", take_a_word(back_talk));
        return 名言;
    }

    public static String 来点Discuss(String 主题) {
        String 句子 = take_a_word(Discuss);
        //      句子 = 句子.replace(RegExp("主题", "g"),主题);
        句子 = 句子.replace("主题", 主题);
        return 句子;
    }

    public static String add_paragraph(String chapter) {
        if (chapter.endsWith(" ")) {
            chapter = chapter.substring(0, chapter.length() - 2);
        }
    ...
    public static String Generate_articles(String 主题) {
        return Generate_articles(主题, 4000);
    }

    public static String Generate_articles(String 主题, Integer wenzanzhishu) {
        List<String> article = new ArrayList<>();
        //      for(String 空 : 主题){
        String chapter = "";
        Integer chapterlen = 0;
        while (chapterlen < wenzanzhishu) {
            Integer 随机数 = take_any_number(null, null);
            if (随机数 < 5 && chapter.length() > 200) {
                chapter = add_paragraph(chapter);
                article.add(chapter);
                chapter = "";
            } else if (随机数 < 20) {
                String 句子 = 来点famous_quotes();
                chapterlen = chapterlen + 句子.length();
                chapter = chapter + 句子;
            } else {
                String 句子 = 来点Discuss(主题);
                chapterlen = chapterlen + 句子.length();
                chapter = chapter + 句子;
            }
        }
